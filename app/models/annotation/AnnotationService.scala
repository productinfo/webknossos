...
class AnnotationService @Inject()(annotationInformationProvider: AnnotationInformationProvider,
  ...
  private def selectSuitableTeam(user: User, dataSet: DataSet)(implicit ctx: DBAccessContext): Fox[ObjectId] =
    (for {
      ...
      val selectedTeamOpt =
        datasetAllowedTeamIds.intersect(userTeamIds).headOption
  ...
  def createExplorationalFor(user: User, _dataSet: ObjectId, tracingType: TracingType.Value, withFallback: Boolean)(
    ...
    def createTracings(dataSet: DataSet, dataSource: DataSource): Fox[(Option[String], Option[String])] =
      tracingType match {
        case TracingType.skeleton =>
          for {
            handler <- dataSetService.handlerFor(dataSet)
            skeletonTracingId <- handler.saveSkeletonTracing(
  ...
  def abortInitializedAnnotationOnFailure(initializingAnnotationId: ObjectId, insertedAnnotationBox: Box[Annotation]) =
    insertedAnnotationBox match {
      ...
      case _ =>
        annotationDAO.abortInitializingAnnotation(initializingAnnotationId)
    }
  ...
  private def getTracingsScalesAndNamesFor(annotations: List[Annotation])(implicit ctx: DBAccessContext): Fox[List[
    ...
    val annotationsGrouped: Map[ObjectId, List[Annotation]] =
      annotations.groupBy(_._dataSet)
  ...
  def resetToBase(annotation: Annotation)(implicit ctx: DBAccessContext, m: MessagesProvider) =
