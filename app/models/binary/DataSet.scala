...
class DataSetDAO @Inject()(sqlClient: SQLClient,
  ...
  private def parseScaleOpt(literalOpt: Option[String]): Fox[Option[Scale]] =
    literalOpt match {
      case Some(literal) =>
        for {
  ...
  def insertOne(d: DataSet)(implicit ctx: DBAccessContext): Fox[Unit] = {
    val defaultConfiguration: Option[String] =
      d.defaultConfiguration.map(c => Json.toJson(c.configuration).toString)
    for {
  ...
  def deactivateUnreported(names: List[String], dataStoreName: String): Fox[Unit] = {
    val inclusionPredicate =
      ...
      else s"name not in ${writeStructTupleWithQuotes(names.map(sanitize))}"
...
class DataSetResolutionsDAO @Inject()(sqlClient: SQLClient)(implicit ec: ExecutionContext)
  ...
  def updateResolutions(_dataSet: ObjectId, dataLayersOpt: Option[List[DataLayer]]): Fox[Unit] = {
    val clearQuery =
      sqlu"delete from webknossos.dataSet_resolutions where _dataSet = ${_dataSet.id}"
