...
class DataStoreHandler(dataStore: DataStore, dataSet: DataSet, rpc: RPC)(implicit ec: ExecutionContext)
    extends LazyLogging {
  ...
  private def urlEncode(text: String) =
    UriEncoding.encodePathSegment(text, "UTF-8")
