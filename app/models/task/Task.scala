...
class TaskDAO @Inject()(sqlClient: SQLClient, projectDAO: ProjectDAO)(implicit ec: ExecutionContext)
  ...
  def findAllByProjectAndTaskTypeAndIdsAndUser(
    ...
    val projectFilterFox = projectNameOpt match {
      case Some(pName) =>
        for { project <- projectDAO.findOneByName(pName) } yield s"(t._project = '${project._id}')"
      case _ => Fox.successful("true")
    }
    val taskTypeFilter =
      taskTypeIdOpt.map(ttId => s"(t._taskType = '$ttId')").getOrElse("true")
    val taskIdsFilter = taskIdsOpt
      .map(
        tIds =>
          if (tIds.isEmpty) "false"
          else s"(t._id in ${writeStructTupleWithQuotes(tIds.map(_.toString))})")
      .getOrElse("true")
