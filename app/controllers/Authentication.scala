...
class Authentication @Inject()(actorSystem: ActorSystem,
  ...
  val combinedAuthenticatorService =
    wkSilhouetteEnvironment.combinedAuthenticatorService
  val bearerTokenAuthenticatorService =
    combinedAuthenticatorService.tokenAuthenticatorService
          ...
          .flatMap {
            loginInfo =>
              userService.retrieve(loginInfo).flatMap {
                ...
                case Some(user) =>
                  Future.successful(BadRequest(Messages("user.deactivated")))
              }
          }
          .recover {
  ...
  def changePassword = sil.SecuredAction.async { implicit request =>
    changePasswordForm.bindFromRequest.fold(
      ...
      passwords => {
        val credentials =
          Credentials(request.identity.email, passwords.oldPassword)
        credentialsProvider
          .authenticate(credentials)
          .flatMap {
            loginInfo =>
              userService.retrieve(loginInfo).flatMap {
                ...
                case Some(user) =>
                  val loginInfo =
                    LoginInfo(CredentialsProvider.ID, request.identity.email)
                  for {
  ...
  def logout = sil.UserAwareAction.async { implicit request =>
    request.authenticator match {
      case Some(authenticator) =>
        combinedAuthenticatorService.discard(authenticator, Ok)
      case _ => Future.successful(Ok)
    }
  }
  ...
  def singleSignOn(sso: String, sig: String) = sil.UserAwareAction.async { implicit request =>
    ...
    request.identity match {
      case Some(user) =>
        ...
        if (HmacUtils.hmacSha256Hex(ssoKey, sso) == sig) {
          ...
          for {
            ...
            val encodedReturnPayload =
              Base64.encodeBase64String(returnPayload.getBytes("UTF-8"))
            val returnSignature =
              HmacUtils.hmacSha256Hex(ssoKey, encodedReturnPayload)
      ...
      case _ =>
        Fox.successful(Redirect("/auth/login?redirectPage=http://discuss.webknossos.org")) // not logged in
    }
  }
