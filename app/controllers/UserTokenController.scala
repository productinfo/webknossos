...
class UserTokenController @Inject()(dataSetDAO: DataSetDAO,
  ...
  def validateUserAccess(name: String, token: String) =
  ...
  private def handleTracingAccess(tracingId: String, mode: AccessMode.Value, userBox: Box[User])(
    ...
    for {
      ...
      if (allowed) UserAccessAnswer(true)
      else
        UserAccessAnswer(false, Some(s"No ${mode.toString} access to tracing"))
    }
  }
}
