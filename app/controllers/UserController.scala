...
class UserController @Inject()(userService: UserService,
  ...
  def annotations(isFinished: Option[Boolean], limit: Option[Int]) =
  ...
  def tasks(isFinished: Option[Boolean], limit: Option[Int]) =
    sil.SecuredAction.async { implicit request =>
      for {
        annotations <- annotationDAO.findAllFor(request.identity._id,
                                                isFinished,
    ...
    UsingFilters(
      Filter("isEditable",
             (value: Boolean, el: User) =>
               for {
                 isEditable <- userService.isEditableBy(el, request.identity)
               } yield isEditable == value),
    ...
    withJsonBodyUsing(userUpdateReader) {
      case (firstName, lastName, email, isActive, isAdmin, assignedMemberships, experiences) =>
        for {
          ...
          trimmedExperiences = experiences.map {
            case (key, value) => key.trim -> value
          }
