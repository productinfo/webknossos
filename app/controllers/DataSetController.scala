...
class DataSetController @Inject()(userService: UserService,
  ...
  def thumbnail(organizationName: String, dataSetName: String, dataLayerName: String, w: Option[Int], h: Option[Int]) =
    sil.UserAwareAction.async { implicit request =>
      def imageFromCacheIfPossible(dataSet: DataSet): Fox[Array[Byte]] = {
        val width =
          Math.clamp(w.getOrElse(DefaultThumbnailWidth), 1, MaxThumbnailHeight)
        val height =
          Math.clamp(h.getOrElse(DefaultThumbnailHeight), 1, MaxThumbnailHeight)
    ...
    UsingFilters(
      Filter(
        ...
        (value: Boolean, el: DataSet) =>
          for {
            isEditable <- dataSetService.isEditableBy(el, request.identity)
  ...
  def accessList(organizationName: String, dataSetName: String) =
  ...
  def update(organizationName: String, dataSetName: String) =
  ...
  def updateTeams(organizationName: String, dataSetName: String) =
    sil.SecuredAction.async(parse.json) { implicit request =>
  ...
  def getSharingToken(organizationName: String, dataSetName: String) =
    sil.SecuredAction.async { implicit request =>
      for {
  ...
  def deleteSharingToken(organizationName: String, dataSetName: String) =
    sil.SecuredAction.async { implicit request =>
      for {
  ...
  def isValidNewName(organizationName: String, dataSetName: String) =
    sil.SecuredAction.async { implicit request =>
      for {
  ...
  def getOrganizationForDataSet(dataSetName: String) =
    sil.UserAwareAction.async { implicit request =>
      for {
