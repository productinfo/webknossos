...
class AnnotationController @Inject()(
  ...
  def merge(typ: String, id: String, mergedTyp: String, mergedId: String) =
  ...
  object CreateExplorationalParameters {
    implicit val jsonFormat = Json.format[CreateExplorationalParameters]
  }
  ...
  def editAnnotation(typ: String, id: String) =
  ...
  private def duplicateAnnotation(annotation: Annotation, user: User)(implicit ctx: DBAccessContext,
    ...
    for {
      ...
      newSkeletonTracingReference <- Fox.runOptional(annotation.skeletonTracingId)(id =>
        dataStoreHandler.duplicateSkeletonTracing(id)) ?~> "Failed to duplicate skeleton tracing."
      newVolumeTracingReference <- Fox.runOptional(annotation.volumeTracingId)(id =>
        dataStoreHandler.duplicateVolumeTracing(id)) ?~> "Failed to duplicate volume tracing."
