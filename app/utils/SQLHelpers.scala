...
class SQLClient @Inject()(configuration: Configuration) {
  lazy val db: PostgresProfile.backend.Database =
    Database.forConfig("slick.db", configuration.underlying)
}
...
object ObjectId extends FoxImplicits {
  ...
  private def parseSync(input: String) =
...
abstract class SecuredSQLDAO @Inject()(sqlClient: SQLClient)(implicit ec: ExecutionContext)
  ...
  def updateAccessQ(requestingUserId: ObjectId): String =
    readAccessQ(requestingUserId)
  def deleteAccessQ(requestingUserId: ObjectId): String =
    readAccessQ(requestingUserId)
  ...
  def readAccessQuery(implicit ctx: DBAccessContext): Fox[String] =
    if (ctx.globalAccess) Fox.successful("true")
      ...
      for {
        ...
        userIdBox match {
          case Full(userId) =>
            "(" + readAccessFromUserOrToken(userId, sharingTokenFromCtx)(ctx) + ")"
  ...
  private def sharingTokenFromCtx(implicit ctx: DBAccessContext): Option[String] =
    ctx.data match {
      case Some(sharingTokenContainer: SharingTokenContainer) =>
        Some(sanitize(sharingTokenContainer.sharingToken))
      case Some(userSharingTokenContainer: UserSharingTokenContainer) =>
  ...
  private def readAccessFromUserOrToken(userId: ObjectId, tokenOption: Option[String])(
    ...
    tokenOption match {
      case Some(_) =>
        "((" + anonymousReadAccessQ(sharingTokenFromCtx) + ") OR (" + readAccessQ(userId) + "))"
...
abstract class SQLDAO[C, R, X <: AbstractTable[R]] @Inject()(sqlClient: SQLClient)(implicit ec: ExecutionContext)
  ...
  def columnsWithPrefix(prefix: String) =
    columnsList.map(prefix + _).mkString(", ")
  ...
  def deleteOne(id: ObjectId)(implicit ctx: DBAccessContext): Fox[Unit] = {
    val q = for {
      row <- collection if notdel(row) && idColumn(row) === id.id
