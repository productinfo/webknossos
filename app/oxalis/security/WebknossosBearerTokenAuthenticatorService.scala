...
class WebknossosBearerTokenAuthenticatorService(settings: BearerTokenAuthenticatorSettings,
  ...
  val dataStoreExpiry =
    conf.Silhouette.TokenAuthenticator.dataStoreExpiry.toMillis millis
  ...
  def create(loginInfo: LoginInfo, tokenType: TokenType)(
    ...
    val expiry: Duration = tokenType match {
      ...
      case _ =>
        throw new Exception("Cannot create an authenticator without a valid TokenType")
    }
    idGenerator.generate.map { id =>
      ...
      case e =>
        throw new AuthenticatorCreationException(CreateError.format(ID, loginInfo), e)
    }
  }
