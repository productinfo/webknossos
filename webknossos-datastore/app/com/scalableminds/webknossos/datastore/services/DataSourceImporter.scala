...
trait DataSourceImporter {
  ...
  def exploreDataSource(id: DataSourceId,
    ...
    PathUtils.listDirectories(baseDir).map { layerDirs =>
      ...
      GenericDataSource(id, layers, previous.map(_.scale).getOrElse(Scale.default), getDataSourceFileSize(baseDir))
    }
  ...
  protected def getDataSourceFileSize(baseDir: Path): Option[Long] =
    try {
      val lineStream = Process(List("du", "-sk"), baseDir.toFile).lineStream
      val returnString = lineStream.mkString
      val index = returnString.indexOf('.')
      val size = returnString.substring(0, index - 1)
      Some(size.toLong * 1000)
    } catch {
      case _ => None
    }
